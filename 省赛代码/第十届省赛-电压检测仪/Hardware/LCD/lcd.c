/**
  ...
  *
  * 在例程的基础上增加了 LCD_Init();
	* 由于LCD也用到了GPIOC_PIN_8~GPIOC_PIN_15，所以LCD和LED会产生冲突，那么我们只需要在LCD的函数LCD_WriteRAM_Prepare()前后加上
	* u16 temp_lcd = GPIOC->ODR; GPIOC->ODR = temp_lcd;防止LCD与LED冲突
  *
  *************************************************************************************************************************
  */
...
* Description    : Initializes the LCD.初始化LCD
* Input          : None
* Output         : None
* Return         : None
*******************************************************************************/
...
* Description    : Sets the Background color.设置背景色。
...
* Description    : Clears the selected line.清除选定行。
...
* Description    : Clears the hole LCD.清除孔LCD。
...
* Description    : Draws a character on LCD.在LCD上绘制字符。
...
* Description    : Displays one character (16dots width, 24dots height).显示一个字符（16点宽，24点高）
...
* Description    : Displays a maximum of 20 char on the LCD.在LCD上最多显示20个字符
* Input          : - Line: the Line where to display the character shape .
*                    This parameter can be one of the following values:
*                       - Linex: where x can be 0..9
...
* Description    : Displays a line.显示一行。
* Input          : - Xpos: specifies the X position.
*                  - Ypos: specifies the Y position.
*                  - Length: line length.
...
* Description    : Displays a rectangle.显示矩形。
* Input          : - Xpos: specifies the X position.
*                  - Ypos: specifies the Y position.
...
* Description    : Displays a monocolor picture.显示单色图片
...
* Description    : Displays a bitmap picture loaded in the internal Flash.显示内部闪存中加载的位图图片。
...
* Description    : Writes to the selected LCD register.写入选定的LCD寄存器。
...
* Description    : Reads the selected LCD Register.读取所选LCD寄存器。
* Input          : None
* Output         : None
