...
volatile uint16_t CCR1_Val = 1000000 / 100;
volatile uint16_t CCR2_Val = 1000000 / 200;
...
{
	static	_Bool PA6_Flag = 0;
	static	_Bool PA7_Flag = 0;
	uint16_t capture = 0;
  ...
  if (TIM_GetITStatus(TIM3, TIM_IT_CC1) != RESET)
  {
    ...
    capture = TIM_GetCapture1(TIM3);
		if(PA6_Flag)
			...
			TIM_SetCompare1(TIM3, capture + CCR1_Val * (1.00 - PA6_Duty) );
		PA6_Flag = !PA6_Flag;
  }
  ...
  if (TIM_GetITStatus(TIM3, TIM_IT_CC2) != RESET)
  {
    ...
    capture = TIM_GetCapture2(TIM3);
		if(PA7_Flag)
			...
			TIM_SetCompare2(TIM3, capture + CCR2_Val * (1.00 - PA7_Duty) );
		PA7_Flag = !PA7_Flag;
  }
}
